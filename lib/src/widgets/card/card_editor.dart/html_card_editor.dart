...
class HtmlCardEditor extends StatefulWidget implements CardEditorInterface {
  ...
  Future<void> createOrUpdateCard(int groupId, {String? path}) async {
    await createCardInDb(groupId, CardDisplayerType.html, path, HTMLContentsCompanion.insert(recto: drift.Value(rectoController.text), verso: drift.Value(versoController.text), cardTemplatedJson: const drift.Value(""), isTemplated: const drift.Value(false)));
  }
